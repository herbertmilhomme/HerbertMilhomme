...
class Page {
    ...
    public static function getPageByID(pageID, isActive = true){
        ...
        if(isActive){
            if(result["status"] == Page.STATUS_ACTIVE){
                return result;
            }else{
                return;
            }
    ...
    /// <returns></returns> array|Indexed|void
    /// </summery>
    ...
    public function deletePageByID(pageID, userID = null){
        ...
        global db;
        postIns = new Post();
        pageFollowerIns = new PageFollower();
        ...
        if(!empty(userID) && pageData["userID"] != userID && !is_admin()){
    ...
    /// 
    /// <typeparam name=""></typeparam> integer userID
    /// <typeparam name=""></typeparam> integer status : value will be one of (STATUS_INACTIVE, STATUS_ACTIVE)
    /// <returns></returns> bool|void
    /// </summery>
    public function massStatusChange(userID, status = Page.STATUS_INACTIVE){
        ...
        query = "";
        if(status == Page.STATUS_INACTIVE){

            // To make inactive from active
            query = sprintf("UPDATE %s SET STATUS=%d WHERE STATUS=%d AND userID=%d", TABLE_PAGES, Page.STATUS_INACTIVE, Page.STATUS_ACTIVE, userID);
        }else if(status == Page.STATUS_ACTIVE){

            // To make active from inactive
            query = sprintf("UPDATE %s SET STATUS=%d WHERE STATUS=%d AND userID=%d", TABLE_PAGES, Page.STATUS_ACTIVE, Page.STATUS_INACTIVE, userID);
        }else{
            //Error
            return;
        }
