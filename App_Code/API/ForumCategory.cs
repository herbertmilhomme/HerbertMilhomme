...
class ForumCategory {
    ...
    /// 
    /// <typeparam name=""></typeparam> Int catID
    /// </summery>
    ...
    public function getCategoryDescription(categoryID){
        ...
        switch(categoryID){
            case 43:
                ...
                break;
            case 45:
                catDesc = "Just joined the website? Feel free to introduce yourself here. Oh, by the way, welcome to " + SITE_NAME + "!";
                break;
            case 6:
    ...
    /// <summery>
    /// <typeparam name=""></typeparam> catID
    /// <returns></returns> array
    /// </summery>
    public static function getCategoryHierarchical(catID){
        ...
        cCat = ForumCategory.getCategory(catID);
        result[] = cCat;
        while(cCat && cCat["parentID"] != 0){
            cCat = ForumCategory.getCategory(cCat["parentID"]);
            result[] = cCat;
        }
    ...
    /// <typeparam name=""></typeparam> description
    ...
    public static function saveCategory(id, name, description){
        ...
        if(!id){ //New Category
            ...
            if(!id){
                add_message(db.last_error, MSG_TYPE_ERROR);
                return false;
            }

            //Make the user to follow this forum
            ForumFollower.followForum(userID, id);
        }else{
    ...
    /// <typeparam name=""></typeparam> int x
    /// <typeparam name=""></typeparam> int y
    /// <typeparam name=""></typeparam>     size
    ...
    public static function saveForumImage(id, file, x = 0, y = 0, size){
        ...
        //Create Logo File
        resize_image(sourceFile, destFile, destType, 350, 350, x, y, size, size);
        resize_image(sourceFile, destFile1, destType, 30, 30, x, y, size, size);
    ...
    /// <summery>
    /// <typeparam name=""></typeparam> keyword
    /// <typeparam name=""></typeparam> page
    /// <typeparam name=""></typeparam> orderBy
    /// <typeparam name=""></typeparam> limit
    /// <returns></returns> array
    /// </summery>
    ...
    public static function deleteCategory(categoryID){
        ...
        foreach(topics as tRow){
            ForumTopic.deleteTopic(tRow["topicID"]);
        }
