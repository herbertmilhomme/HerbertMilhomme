// <!--
	...
	var Handler = {
		...
		enable: function (val) {
			if (val === true) {
				...
				$("#chkGranted").addClass("active");
				$("#chkDenied").removeClass("active");
				$("input[name='permission']").attr("disabled", true);
				...
				$("#chkDenied").addClass("active");
				$("#chkGranted").removeClass("active");
	...
	/*$("#frmMain").submit(*/function msgNotification (title,message,icon,onClickURL) {//event
		if (onClickURL !== null) {
			// Let's check if the user is okay to get some notification
			if (Notification.permission === "granted") {
				// If it's okay let's create a notification
				Handler.displayNotification(
					title,//"#conversation-list li [msgTAGcount] .conversation-text [0] i"//$("#txtTitle").val(),
					message,
					icon, //=== "1" ? $("#txtUrl").val() : ($("#selIcon").val() === "2" ? $("#hidFile").val() : ""),
					0, //$("#txtSeconds").val()
					//If user clicks the notification a new page/tab opens up.
					new Function('window.open("' + onClickURL + '", "_blank")')
				);
			} else {
	...
	$("#frmMain").submit(function (event) {
		/*if ($(this).data("action") === "code") {
			...
			if (Notification.permission === "granted") {
				// If it's okay let's create a notification
				Handler.displayNotification(
					$("#txtTitle").val(),//"#conversation-list li [msgTAGcount] .conversation-text [0] i"//$("#txtTitle").val(),
					$("#txtMessage").val(),
